...
class CUDAPlatform(Platform):
    ...
    def get_attention_backend_cls(cls, selected_backend: _Backend):
        ...
        if selected_backend == _Backend.NATIVE_ATTN:
            ...
            logger.info("Using TRTLLM (NoReorder) ATTN backend.")
            return "fastdeploy.model_executor.layers.attention.TrtllmNoReorderAttentionBackend"
        else:
            raise ValueError(
                "Invalid attention backend you specified.\n"
                "Now only support [NATIVE_ATTN, MLA_ATTN, APPEND_ATTN, FLASH_ATTN, FLASH_MASK_ATTN, TRTLLM_ATTN] in cuda place."
            )
